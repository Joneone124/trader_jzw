...
def alert(cate, rate_open, rate_long):
    xls = pd.DataFrame({'币种': cate, '波动幅度': rate_open, '多空比': rate_long}, index=None)

    plt.rcParams['font.sans-serif'] = ['Microsoft YaHei']  # 显示中文
    fig = plt.figure(figsize=(6, 10), dpi=300)  # 图片的长、宽、像素
    ax = fig.add_subplot(111, frame_on=False)  # 111表示只画一张图
    ax.xaxis.set_visible(False)
    ax.yaxis.set_visible(False)
    table(ax, xls, loc='center')
    plt.savefig('pic_open_long.jpg')
    ...
    file_image = open('pic_open_long.jpg', 'rb').read()
